...
struct thread
  {
    ...
    int init_priority; // Initial priority of the donation recipient before donation.
    struct list donators;  // List of threads that donated.
    struct list_elem donator_elem; // Donation list's element.
    struct lock *wait_on_lock; // Current lock that the thread is trying to get.
...
bool thread_compare_donator_priority (const struct list_elem *e, const struct list_elem *f, void *aux UNUSED);
void priority_donate (void);
void remove_donator_with_lock (struct lock *lock);
